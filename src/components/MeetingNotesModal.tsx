...
const MeetingNotesModal = ({ open, onClose, clients, onUpdateClient, onAddClient }: MeetingNotesModalProps) => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setError(e?.message || "Error processing. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="fixed inset-4 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-3xl md:max-h-[85vh] bg-card rounded-xl shadow-modal z-50 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-base">Process Internal Meeting</h2>
          </div>
          ...
          {!result ? (
            <>
              <p className="text-sm text-muted-foreground">
                Paste the transcript or summary of an internal meeting. The AI will identify mentioned companies,
                next steps, and relevant updates.
              </p>
              <textarea
                value={transcript}
                onChange={(e) => setTranscript(e.target.value)}
                placeholder="Paste your internal meeting transcript or summary here..."
            ...
            <>
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-foreground">
                  {result.insights.length} compan{result.insights.length !== 1 ? "ies" : "y"} identified
                </p>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            {insight.urgency && (
                              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                                ...
                                {insight.urgency === "Alta" ? "High" : insight.urgency === "Média" ? "Medium" : "Low"}
                              </span>
                            )}
                          </div>
                          ...
                          {insight.nextSteps.length > 0 && (
                            <div className="space-y-1 mb-2">
                              <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Next steps:</p>
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                            {insight.deadline && (
                              <span>Deadline: {new Date(insight.deadline).toLocaleDateString("en-US")}</span>
                            )}
                            {insight.suggestedStage && (
                              <span className="flex items-center gap-1">
                                Stage: <strong className="text-foreground">{stageLabel(insight.suggestedStage as any)}</strong>
                              </span>
                            )}
                            {insight.responsible && (
                              <span>Owner: {insight.responsible}</span>
                            )}
                          </div>
                        </div>
              ...
              {result.generalNotes && (
                <div className="rounded-lg border border-border bg-secondary/50 p-3">
                  <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1">General Notes</p>
